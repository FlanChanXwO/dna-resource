...
根目录 `version` 只能是正整数。任何资源、别名、兑换码、字体、纹理或文档变更都必须提高版本，并将 `resource_manifest.json.resource_version` 同步为同一个数字字符串。提交前运行 `python3 scripts/check_resource_version.py --base <base-ref> --head <head-ref>`。首次迁移的 head 必须为 `1`；之后不得删除 `version`。
...
历史素材保留原有尺寸基线 2460 宽；新角色/武器图鉴一律从皎皎角抓取。
